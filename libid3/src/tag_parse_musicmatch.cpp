...
namespace 
{
  uint32 readSeconds(ID3_Reader& reader, size_t len)
  {
    ...
    io::WindowedReader wr(reader, (uint32)len);
...
//    ID3D_NOTICE( "mm::parse: bailing, not enough bytes to parse offsets, pos = " << end );
  ...
  else
  {
    ...
    for (size_t i = 0; i < sizeof(possibleSizes)/sizeof(size_t); ++i)
    {
      ...
      dataWindow.setCur((uint32)(dataWindow.getCur() - offset));
  ...
  if (0 == metadataSize)
  {
      ...
      printf( "mm::parse: bailing, couldn't find meta data signature");//, end = " << end );
      return false;
  }
  ...
  if (dataWindow.getEnd() < tagSize)
  {
      ...
      // the file.  Bail!
      printf( "mm::parse: bailing, tag size is too big, tag size = ");// << tagSize << ", end = " << end );
      return false;
  }

  dataWindow.setBeg((uint32)(dataWindow.getEnd() - tagSize));
  ...
  {
    offsets[i+1] = (uint32)(offsets[i] + sectionSizes[i]);
  }

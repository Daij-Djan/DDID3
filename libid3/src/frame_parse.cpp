    ...
    for (ID3_FrameImpl::iterator fi = frame.begin(); fi != frame.end(); ++fi)
    {
      ...
      { 
        // there's no remaining data to parse! 
          printf( "ID3_FrameImpl::Parse(): out of data");// at postion " <<
//                      rdr.getCur() );
        if (iLoop == iFields)
        {
      ...
      if (NULL == fp)
      {
        ...
        printf( "ID3_FrameImpl::Parse(): field is null" );
        continue;
      }
...
//                   rdr.getEnd() );
      ...
      { 
        ...
        printf( "ID3_FrameImpl::Parse(): no data parsed, bad parse" );
        return false;
      }
...
//  ID3D_NOTICE( "ID3_FrameImpl::Parse(): reader.getEnd() = " << reader.getEnd() );
  ...
  { 
    printf( "ID3_FrameImpl::Parse(): no header to parse" );
    return false; 
  }
...
//  ID3D_NOTICE( "ID3_FrameImpl::Parse(): dataSize = " << dataSize );
  ...
  {
    printf( "ID3_FrameImpl::Parse(): not enough data to parse frame" );
    return false;
  }
  io::WindowedReader wr(reader, (uint32)dataSize);
...
//    ID3D_NOTICE( "ID3_FrameImpl::Parse(): frame is encrypted, grouping_id = " << (int) ch );
  ...
  {
    io::CompressedReader csr(wr, (uint32)origSize);

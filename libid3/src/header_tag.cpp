...
{
  writer.writeChars((uchar *) ID, (uint32)strlen(ID));
  ...
  io::writeUInt28(writer, (uint32)this->GetDataSize()); //now includes the extended header
...
bool ID3_TagHeader::Parse(ID3_Reader& reader)
{
  ...
  {
    printf( "ID3_TagHeader::Parse(): not an id3v2 header" );
    return false;
  }

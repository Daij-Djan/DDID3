...
ID3_Reader::pos_type io::WindowedReader::setBeg(pos_type beg)
{
  ...
  {
      printf( "WindowedReader::setBeg() failed" ); //, [beg, _end] = " << beg << ", " << this->getEnd() << "]" );
  }
  else
  {
      printf( "WindowedReader::setBeg() failed" ); //, [beg, _beg] = " << beg << ", " << this->getBeg() << "]" );
  }
  return _beg;
}
...
ID3_Reader::pos_type io::WindowedReader::setEnd(pos_type end)
{
  ...
  {
      printf( "WindowedReader::setEnd() failed" ); //, end = " << end );
...
ID3_Reader::int_type io::WindowedReader::readChar()
{
  ...
  {
      printf( "io::WindowedReader::readChar: not in window"); // <<
...
void io::CompressedWriter::flush()
{
  ...
  size_type dataSize = (size_type)_data.size();
  _origSize = dataSize;
  ...
  {
    // log this
    printf("io::CompressedWriter: error compressing");
...
//    ID3D_NOTICE("io::CompressedWriter: compressed size = " << newDataSize << ", original size = " << dataSize ); 
    _writer.writeChars(newData, (size_type)newDataSize);
  }
  else
  {

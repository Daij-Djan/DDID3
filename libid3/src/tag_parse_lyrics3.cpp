...
//    ID3D_NOTICE( "readTimeStamp(): timestamp = " << sec );
  ...
  bool findText(ID3_Reader& reader, String text)
  {
    ...
    while (!reader.atEnd())
    {
      ...
      if (index == text.size())
      {
        reader.setCur((uint32)(reader.getCur() - index));
...
//      ID3D_NOTICE( "lyrics3toSylt: ms = " << ms );
      ...
      io::writeBENumber(writer, (uint32)ms, sizeof(uint32));
      if (lf)
      {
...
//    ID3D_NOTICE( "id3::v1::parse: bailing, not enough bytes to parse, pos = " << end );
  ...
  if (end < reader.getBeg() + 11 + 9 + 128)
  {
    ...
    printf( "id3::v1::parse: not enough data to parse lyrics3" );
    return false;
  }
  ...
  reader.setCur((uint32)(end - lyrDataSize));
  io::WindowedReader wr(reader, (uint32)(lyrDataSize - (9 + 128)));

  if (!findText(wr, "LYRICSBEGIN"))
  {
    printf( "id3::v1::parse: couldn't find LYRICSBEGIN, bailing" );
    return false;
  }
...
//                 lyrSize );
  ...
  {
      printf( "lyr3::v2::parse: not enough data to parse tag, lyrSize = ");// << lyrSize );
      return false;
  }
  ...
  if (io::readText(wr, 11) != "LYRICSBEGIN")
  {
    ...
    printf( "lyr3::v2::parse: couldn't find LYRICSBEGIN, bailing" );
    return false;
  }
...
//        ID3D_NOTICE( "lyr3::v2::parse: determined synced lyrics" );
    ...
    else if (fldName == "IMG")
    {
      ...
      printf( "lyr3::v2::parse: IMG field unsupported" );
    }
    else
    {
        printf( "lyr3::v2::parse: undefined field id ");// <<
//                    fldName );
    }
  }

...
//      ID3D_NOTICE( "id3::v2::parseFrames(): frameSize = " << frameSize );
      ...
      if (frameSize == 0)
      {
        ...
        printf( "id3::v2::parseFrames(): frame size is 0, can't continue parsing frames");
        delete f;
        // Break for now.
        break;
      }
      else if (!goodParse)
      {
        ...
        printf( "id3::v2::parseFrames(): bad parse, deleting frame");
        delete f;
      }
...
//                     "compressed frame");
        ...
        if (fld)
        {
          ID3_MemoryReader mr(fld->GetRawBinary(), (uint32)fld->BinSize());
          ID3_Reader::char_type ch = mr.readChar();
          if (ch != 'z')
          {
            // unknown compression method
              printf( "id3::v2::parseFrames(): unknown compression id ");// <<
//                          " = '" << ch << "'" );
          }
          else
          {
...
//            size_t oldSize = f->GetDataSize() - sizeof(uint32) - 1;
            ...
            if (!cr.atEnd())
            {
              ...
              // why.
                printf( "id3::v2::parseFrames(): didn't parse entire ");// <<
...
//  ID3D_NOTICE( "ID3_TagImpl::Parse(ID3_Reader&): dataSize = " << dataSize);
  ...
  wr.setWindow(wr.getCur(), (uint32)dataSize);
...
//        ID3D_NOTICE( "ID3_TagImpl::ParseReader(): id3v1! cur = " << wr.getCur() );
    ...
    { //it has at least the size for a mp3 header (a mp3 header is 4 bytes)
      wr.setBeg((uint32)(_prepended_bytes + bytes_till_sync));
      wr.setCur((uint32)(_prepended_bytes + bytes_till_sync));
      wr.setEnd((uint32)(_file_size - _appended_bytes));
